...
## Significance test for average distance from health post for each catchment area

```{r}
library(tidyr)
library(broom)
fit <- lm(distance ~ health_post, data = census_spatial@data)
var <- anova(fit)
print(var)
...
ci <-  tidy(confint(fit))
names(ci)[1] <- 'term'
result <- left_join(fit_tidy, ci)
result$term <- gsub('health_postCentro de saude de ', '', result$term)
result$term <- factor(result$term, levels = result$term)
...
t.test(x = census_spatial@data$distance)
```
...
#                 latitude,
#                 gender)
# write_csv(x, '~/Desktop/census_data_for_denise.csv')
```

# Estimated HIV numbers by health post

```{r}
# Prevalence by health post
x <- census_spatial@data %>%
  group_by(health_post) %>%
  summarise(people = n(),
            denominator = length(which(!is.na(prevalence))),
            prevalence = mean(prevalence, na.rm = TRUE)) %>%
  mutate(estimated_hiv_people = round(denominator * (prevalence / 100)))
DT::datatable(x)
``` 